...
export default function EditProduct() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        const data = await apiCall(`/api/inventory/${productId}`);
        if (data && data.product_id) {
          // Use first batch for editing
          const batch = Array.isArray(data.batches) && data.batches.length > 0 ? data.batches[0] : {};
          setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setError('');
      const result = await apiCall(`/api/inventory/${productId}`, {
        method: 'PUT',
        body: JSON.stringify({
          ...formData,
